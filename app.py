...
class Usuario(UserMixin, db.Model):
    ...
    tipo = db.Column(db.String(10), nullable=False)  # 'vendedor' (líder) ou 'cliente'
...
@login_required
def meus_produtos():
    # Determinar barraca e permissão
    if current_user.tipo == 'vendedor':
        ...
        if not associacao:
            flash("Você não está associado a nenhuma barraca ativa.")
            return redirect(url_for('index'))
        barraca_id = associacao.barraca_id
        tem_permissao = associacao.pode_criar_produto

    if not tem_permissao:
        flash("Você não tem permissão para gerenciar produtos.")
...
def index():
    ...
    # Verificar se o usuário atual é membro aprovado de alguma barraca
        ...
        if 'nota' in request.form:
            ...
            try:
                ...
                if nota_int < 1 or nota_int > 5:
                    flash("Nota deve ser entre 1 e 5.")
...
@login_required
def dashboard():
    # Determinar se é líder ou membro com acesso a alguma barraca
    if current_user.tipo == 'vendedor':
        barraca_id = current_user.id
        is_lider = True
    else:
        associacao = MembroBarraca.query.filter_by(usuario_id=current_user.id, status='aprovado').first()
        if not associacao:
            flash("Você não está associado a nenhuma barraca ou sua solicitação ainda não foi aprovada.")
            return redirect(url_for('index'))
        barraca_id = associacao.barraca_id
        is_lider = False
...
def confirmar_pedido(id):
    ...
    if usuario_pode_confirmar_pedido(current_user, barraca_id) or current_user.is_admin:
        ...
        flash("Você não tem permissão para confirmar pedidos.")
    return redirect(url_for('dashboard'))

# --- GERENCIAMENTO DE MEMBROS (LÍDER) ---

@app.route("/gerenciar_membros", methods=["GET", "POST"])
@login_required
def gerenciar_membros():
    if current_user.tipo != 'vendedor':
        flash("Apenas líderes de barraca podem gerenciar membros.")
        return redirect(url_for('index'))
    barraca_id = current_user.id
    if request.method == "POST":
        ...
        if acao == "aprovar":
            ...
            db.session.commit()
            flash(f"{membro.usuario.username} aprovado com sucesso!")
        elif acao == "rejeitar":
            db.session.delete(membro)
            db.session.commit()
...
def cadastro():
    ...
    if request.method == "POST":
        ...
        papel = request.form.get("papel")  # 'lider' ou 'membro'
        ...
        if tipo == "vendedor" and papel == "lider":
            ...
            db.session.add(novo)
            db.session.commit()
            flash("Conta de líder criada com sucesso!")
            login_user(novo)
            return redirect(url_for("dashboard"))
            ...
            if not barraca_id:
                flash("Selecione uma barraca para se associar.")
                return redirect(url_for("cadastro"))
            novo = Usuario(
            ...
            db.session.commit()
            flash("Conta criada! Aguarde a aprovação do líder da barraca.")
            login_user(novo)
            return redirect(url_for("index"))
